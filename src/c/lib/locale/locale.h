...
//
// Design notes:
//   * Tables live in flash (const), so activating a language is a pointer swap;
//     the only RAM state is the small bookkeeping below.
//   * A string entry may be NULL — the lookup falls back to the base locale
...
// A language/region the app can render in. All pointers reference flash.
typedef struct {
  ...
  const char *const *strings;   // [string_count]; an entry may be NULL → falls back to the base locale
