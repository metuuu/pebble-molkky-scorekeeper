...
static const char *const EN[STR__COUNT] = {
  ...
  [STR_HELP_THROWING_B    ] = "Take turns throwing the mölkky underarm to knock pins down. Everyone throws from the same line, 3,5 m away. Stand fallen pins back up where they land.",
  [STR_HELP_SCORING_T     ] = "Scoring",
  [STR_HELP_SCORING_B     ] = "One pin down scores that pin's number. Two or more down score the number of pins felled (not their values).",
  ...
  [STR_HELP_WINNING_B     ] = "The first player to hit exactly 50 wins. The round is finished so anyone else who also reaches 50 in the same round shares the win.",
...
static const char *const FI[STR__COUNT] = {
  ...
  [STR_HELP_THROWING_B    ] = "Heittäkää vuorotellen mölkkyä alakautta kaataaksenne keiloja. Kaikki heittävät samalta viivalta 3,5 m päästä. Nostakaa kaatuneet keilat pystyyn siihen, mihin ne jäivät.",
  [STR_HELP_SCORING_T     ] = "Pisteytys",
  [STR_HELP_SCORING_B     ] = "Yksi kaatunut keila antaa keilan numeron verran pisteitä. Kaksi tai useampi antaa kaadettujen keilojen lukumäärän (ei niiden numeroita).",
  ...
  [STR_HELP_WINNING_B     ] = "Ensimmäinen tasan 50 saavuttanut voittaa. Kierros pelataan loppuun, joten jokainen, joka myös saavuttaa 50 samalla kierroksella, jakaa voiton.",
...
// Index 0 is the base locale: it backs missing entries and is the default.
